...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/subject_messages/${id}/`, {
      method: 'GET',
      headers: {
        'Cookie': cookie,
      },
    ...
    console.error('Error fetching subject message:', error);
    return NextResponse.json(
      { error: 'Failed to fetch subject message' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to update subject message' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to delete subject message' },
      { status: 500 }
    );
  }
}
