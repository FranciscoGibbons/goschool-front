...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${BACKEND_URL}/api/v1/selfassessables_responses/${queryString ? `?${queryString}` : ''}`;
    ...
    console.error('Error fetching selfassessable responses:', error);
    return NextResponse.json(
      { error: 'Failed to fetch selfassessable responses' },
      { status: 500 }
    );
  }
}
