...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${BACKEND_URL}/api/v1/assistance/${queryString ? `?${queryString}` : ''}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/assistance/`, {
      method: 'POST',
      headers: {
        'Cookie': cookie,
    ...
    return NextResponse.json(
      { error: 'Failed to create assistance' },
      { status: 500 }
    );
  }
}
